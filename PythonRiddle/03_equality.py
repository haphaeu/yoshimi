...
i=4
while i<n-4:
    flag = (      mess[i-4].lower()==mess[i-4]
	      and mess[i-3].upper()==mess[i-3] 
	      and mess[i-2].upper()==mess[i-2] 
	      and mess[i-1].upper()==mess[i-1] 
	      and mess[i+0].lower()==mess[i+0] 
	      and mess[i+1].upper()==mess[i+1] 
	      and mess[i+2].upper()==mess[i+2] 
	      and mess[i+3].upper()==mess[i+3]
	      and mess[i+4].lower()==mess[i+4]
	    )
    if flag: ocurr.append(i)
    i+=1

for i in ocurr: sys.stdout.write("%s" % mess[i])
