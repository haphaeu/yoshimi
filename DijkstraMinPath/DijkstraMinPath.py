...
class Edge():
# defines and edge, with a length d
# and connected to two vertexes, v1 and v2
# v1 and v2 are just the indexes of a
...
class Vertex():
# defines a vertex and some other properties
...
# sub-function of the dijkstraGraph
# returns the index of the next vertex to
# be worked by the algorithm, which is the
# unvisited vertex with the overall minimum
...
# sub-function of the dijkstraGraph
# for a given current vertex, goes through
# all its still unvisited connected vertexes,
# and check if the path through the current vertex
# is smaller than the path actually stored
# -> does not return anything since updates are done
# directly in the list of vertexes
...
# main Dijkstra algorithm for min path finding
# for a given starting vertex, calculates the minimum
# distances to all the other vertexes.
# these minimum distances are saved in the vertexes
# themselves by accessing vertex[i].distance
# -> does not return anything since updates are done
# directly in the list of vertexes
def dijkstraGraph(vertexes, edges, initialVertex):
    #initial setup
    vertexes[initialVertex].distance = 0
    while sum([_.unvisited for _ in vertexes]): 
...
MyVertexes = [Vertex(MINIMUM, True, -1) for _ in range(NumVertexes)]
...
InitialVertex = 0
dijkstraGraph(MyVertexes, MyEdges, InitialVertex)
...
#show results
print "Minimum distance from vertex", InitialVertex,"to vertex",
