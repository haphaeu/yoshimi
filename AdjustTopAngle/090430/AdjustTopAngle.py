...
#numericamente
    ...
    while nIter<maxIter:
        if abs(HorizDist-H)<=tol:
            ...
            if TopA<=dTA: #evita que TopA sse torne negativo
                dTA=TopA/2.0
            TopA-=dTA
            FlagDiminui=True
        else:
            ...
            FlagDiminui=False
        H = CalcInitCatenary(VertDist,TopA)
        print "Iter: %i, Target: %.2f, Current: %.2f, Top Angle: %.2f" % (nIter, HorizDist, H, TopA)
...
##################################################
# if called from prompt line w parameters
# passed as arguments:
##try:
##    nArg=len(sys.argv)
##    if nArg==3:
...
##########################################################
# if called with control DAT file
