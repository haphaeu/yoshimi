...
def gumbelFit(confidence_Level, sample, results, seedNo, colnames):
    ...
    for row in range(len(sample.columns)-3):
        ...
        for column in range(len(results.major_axis)):
            evalv=sample.ix[c:c+seedNo-1,row+3].tolist()
            sortEvalv = sorted(evalv)
            c = (column+1)*seedNo
            tmp_data[0] = sample.ix[column*seedNo,0]  # Hs
            tmp_data[1] = sample.ix[column*seedNo,1]  # Tp
            tmp_data[2] = sample.ix[column*seedNo,2] # WaveDir
            ...
            #Else, sample is maxima or max absolute and right skewed distribution is to be used.
            ...
            # finally feed tmp_data into the results panel
            # this is done for performance, since item assignment by index in pandas
            # panels is VERY slow...
            results.ix[row, column, :] = tmp_data
    return results
...
def plotProbability(results, sample, colnames, seedNo, confidence_Level,
    ...
    for row in range(len(sample.columns)-3):
        ...
        for column in range(len(results.major_axis)):
            ...
            if (results.items[row] in Objectplot and sample.ix[c-seedNo,2] in PlotWd
                and sample.ix[c-seedNo,0] in PlotHs and sample.ix[c-seedNo,1] in PlotT):
                ...
                betaME = results.ix[row, column, 'beta ME']
                muME = results.ix[row, column, 'mu ME']

                betaMLE = results.ix[row, column, 'beta MLE']
                muMLE = results.ix[row, column, 'mu MLE']

                #First supblot is histograms of observations and the pdf of the fitted distribution
                ...
                if 'Min' in sample.columns[row+3]:
                    ...
                    ae.scatter(sorted(evalv), -np.log(-np.log(1-sampleCDF)),  marker='*', color='k')
                    ...
                    if savepng:
                        aepng.scatter(sorted(evalv), -np.log(-np.log(1-sampleCDF)),  marker='*', color='k')
                    ...
                    for i in range(len(confidence_Level)):
                        ae.plot([xlim[0],xlim[1]],[loglogConf[i],loglogConf[i]],'k--',
                                alpha=0.2)
                        ae.annotate(str(round(confidence_Level[i],4)), xy=(xlim[1],loglogConf[i]),
                                        xytext=(xlim[1], loglogConf[i]))
                        ae.plot([loglogValueME[i],loglogValueME[i]],[ylim[0],loglogConf[i]],'r--')
                        ae.annotate(str(round(loglogValueME[i],2)), xy=(loglogValueME[i],ylim[0]),
                                        xytext=(loglogValueME[i], ylim[0]-2),
                                        arrowprops=dict(arrowstyle="->",color='red'))
                        ae.plot([loglogValueMLE[i],loglogValueMLE[i]],[ylim[0],loglogConf[i]],'b--')
                        ae.annotate(str(round(loglogValueMLE[i],2)), xy=(loglogValueMLE[i],ylim[0]),
                        ...
                        if savepng:
                            aepng.plot([xlim[0],xlim[1]],[loglogConf[i],loglogConf[i]],'k--',
                                       alpha=0.2)
                            aepng.annotate(str(round(confidence_Level[i],4)), xy=(xlim[1],loglogConf[i]),
                                        xytext=(xlim[1], loglogConf[i]))
                            aepng.plot([loglogValueME[i],loglogValueME[i]],[ylim[0],loglogConf[i]],'r--')
                            aepng.annotate(str(round(loglogValueME[i],2)), xy=(loglogValueME[i],ylim[0]),
                                        xytext=(loglogValueME[i], ylim[0]-2),
                                        arrowprops=dict(arrowstyle="->",color='red'))
                            aepng.plot([loglogValueMLE[i],loglogValueMLE[i]],[ylim[0],loglogConf[i]],'b--')
                            aepng.annotate(str(round(loglogValueMLE[i],2)), xy=(loglogValueMLE[i],ylim[0]),
                                        xytext=(loglogValueMLE[i], ylim[0]-1),
                                        arrowprops=dict(arrowstyle="->",color='blue'))

                        sampleIndex = seedNo-(confidence_Level[i])*seedNo
                        if sampleIndex.is_integer():
                            x = sortEvalv[int(sampleIndex)]
                            y = -np.log(-np.log(1-sampleCDF[int(sampleIndex)]))
                            ae.annotate('p'+str(confidence_Level[i])+' = '+str(round(x,2)), xy=(x,y)
                                        ,xytext=(x, y+1.0),
                                        arrowprops=dict(arrowstyle="->",color='black'))
                            if savepng:
                                aepng.annotate('p'+str(confidence_Level[i])+' = '+str(round(x,2)), xy=(x,y)
                                        ,xytext=(x, y+1.0),
                                        arrowprops=dict(arrowstyle="->",color='black'))
                    ...
                    if savepng:
                        aepng.scatter(sorted(evalv), -np.log(-np.log(sampleCDF)),  marker='*', color='k')
                        aepng.plot(a, loglog_cdfME,  'r')
                        aepng.plot(b, loglog_cdfMLE,  'b')
                        aepng.set_ylabel('Cumulative probability')
                        aepng.set_ylim(ylim[0],ylim[1])
                    ...
                    for i in range(len(confidence_Level)):
                        ...
                        if savepng:
                            ...
                            aepng.annotate(str(round(loglogValueME[i],2)), xy=(loglogValueME[i],ylim[0]),
                                        xytext=(loglogValueME[i], ylim[0]-2),
                                        arrowprops=dict(arrowstyle="->",color='red'))
                            aepng.plot([loglogValueMLE[i],loglogValueMLE[i]],[-2,loglogConf[i]],'b--')
                            aepng.annotate(str(round(loglogValueMLE[i],2)), xy=(loglogValueMLE[i],ylim[0]),
                                        xytext=(loglogValueMLE[i], ylim[0]-1),
                                        arrowprops=dict(arrowstyle="->",color='blue'))
                        ...
                        if sampleIndex.is_integer():
                            x = sortEvalv[int(sampleIndex)-1]
                            y = -np.log(-np.log(sampleCDF[int(sampleIndex)-1]))
                            ae.annotate('p'+str(confidence_Level[i])+' = '+str(round(x,2)), xy=(x,y)
                                        ,xytext=(x, y-1.0),
                                        arrowprops=dict(arrowstyle="->",color='black'))
                            if savepng:
                ...
                name = '%s Hs %.2f Tp %d wdir %d' % (sample.columns[row+3], sample.ix[c-seedNo,0],
                                                     sample.ix[c-seedNo,1], sample.ix[c-seedNo,2])
