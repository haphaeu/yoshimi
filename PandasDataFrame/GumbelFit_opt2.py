...
def gumbelFit(confidence_Level, sample, results, seedNo, colnames):
    ...
    for row in range(len(sample.columns)-3):
        ...
        for column in range(len(results.major_axis)):
            ...
            # panels is VERY slow...
            results.iloc[row, column, :] = tmp_data
    return results
...
def plotProbability(results, sample, colnames, seedNo, confidence_Level,
    ...
    for row in range(len(sample.columns)-3):
        ...
        for column in range(len(results.major_axis)):
            ...
            if (results.items[row] in Objectplot and sample.iloc[c-seedNo, 2] in PlotWd and
                ...
                betaME = results.iloc[row, column]['beta ME']
                muME = results.iloc[row, column]['mu ME']

                betaMLE = results.iloc[row, column]['beta MLE']
                muMLE = results.iloc[row, column]['mu MLE']
